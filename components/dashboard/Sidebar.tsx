...
// import { Bell, ChevronDown, ChevronRight, ExternalLink, Plus, Power } from 'lucide-react';
...
//                 item.dropdownMenu && item.dropdownMenu.some((link) => link.href === pathname);
...
export default function Sidebar({
  ...
  return (
    <div
      className={cn(
        'fixed top-0 left-0 h-full w-[220px] lg:w-[280px] border-r bg-background overflow-y-auto',
        isMobile ? 'block z-50 shadow-lg' : 'hidden md:block bg-muted/40',
      )}
    >
      <div className="flex h-full max-h-screen flex-col gap-2">
        <div className="flex flex-shrink-0 h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
                ...
                <div key={i}>
                  {item.dropdown ? (
                    <Collapsible open={isOpen}>
                      ...
                      <CollapsibleContent className="bg-muted/50 rounded mt-1">
                        {item.dropdownMenu?.map((menuItem, j) => (
                          <Link
                            ...
                            className={cn(
                              'mx-4 flex items-center gap-3 rounded-lg px-3 py-2 text-muted-foreground transition-all hover:text-primary hover:bg-muted justify-between text-xs ml-6',
                              pathname === menuItem.href && 'bg-muted text-primary',
                            )}
                          >
                            {menuItem.title}
