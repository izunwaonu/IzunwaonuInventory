  ...
  try {
    return await db.$transaction(async (tx) => {
      ...
      if (
        ...
        return {
          error: 'Invalid organization data. Please provide all required fields.',
          status: 400,
          data: null,
        };
      }
      ...
      const org = await tx.organization.create({
        data: {
          ...
          inventory: Array.isArray(orgData.inventory)
            ? { create: orgData.inventory }
            : { create: [] },
      ...
      if (!emailTemplate) {
        ...
        return {
          error: 'Failed to generate email verification template.',
          status: 500,
          data: null,
        };
      }
...
//   }
export async function sendInvite(data: inviteData) {
  const { email, orgId, orgName, roleId, roleName, locationId, locationName } = data;
  try {
    // Check for existing users
    const existingUserByEmail = await db.user.findUnique({ where: { email } });
    if (existingUserByEmail) {
