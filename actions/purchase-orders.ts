...
export async function createPurchaseOrder(data: {
  ...
  try {
    ...
    // Generate organization-specific PO number
    const lastPO = await db.purchaseOrder.findFirst({
      where: { orgId },
      orderBy: { createdAt: 'desc' },
      select: { poNumber: true },
    ...
    let poNumber = `PO-${orgPrefix}-00001`;
    if (lastPO && lastPO.poNumber) {
      // Check if the last PO number follows the new format
      if (lastPO.poNumber.includes(orgPrefix)) {
        // Extract number from format: PO-ORGX-00001
        const parts = lastPO.poNumber.split('-');
        if (parts.length === 3) {
          const lastNumber = Number.parseInt(parts[2]);
          if (!isNaN(lastNumber)) {
            poNumber = `PO-${orgPrefix}-${String(lastNumber + 1).padStart(5, '0')}`;
          }
        }
      } else {
        // Old format or different organization, check if any PO exists with new format
        const lastNewFormatPO = await db.purchaseOrder.findFirst({
          where: {
            orgId,
            poNumber: {
              startsWith: `PO-${orgPrefix}-`,
            },
          },
          orderBy: { createdAt: 'desc' },
          select: { poNumber: true },
        ...
        if (lastNewFormatPO) {
          const parts = lastNewFormatPO.poNumber.split('-');
          if (parts.length === 3) {
            const lastNumber = Number.parseInt(parts[2]);
            if (!isNaN(lastNumber)) {
              poNumber = `PO-${orgPrefix}-${String(lastNumber + 1).padStart(5, '0')}`;
        ...
        // If no new format PO exists, use the default: PO-ORGX-00001
      }
    }

    console.log(`Generated PO number: ${poNumber} for organization: ${orgId}`);
