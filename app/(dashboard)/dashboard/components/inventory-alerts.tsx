    ...
    <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
      ...
      <CardContent className="p-0">
        <ScrollArea className="h-64 sm:h-72">
          <div className="space-y-2.5 p-3 sm:p-4 pt-0">
            {/* Out of Stock Items */}
            {alerts.outOfStock.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-2.5 p-2.5 bg-red-50 border border-red-200 rounded-lg"
              >
                <Package className="h-3 w-3 text-red-600 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-red-900 truncate">{item.name}</p>
                  <p className="text-xs text-red-700 truncate">SKU: {item.sku}</p>
                  <Badge variant="destructive" className="mt-1 text-xs">
                    Out of Stock
                  </Badge>
                </div>
            ...
            {alerts.lowStock.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-2.5 p-2.5 bg-yellow-50 border border-yellow-200 rounded-lg"
              >
                <AlertTriangle className="h-3 w-3 text-yellow-600 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-yellow-900 truncate">{item.name}</p>
                  <p className="text-xs text-yellow-700 truncate">SKU: {item.sku}</p>
            ...
            {alerts.pendingDeliveries.map((delivery) => (
              <div
                key={delivery.id}
                className="flex items-center gap-2.5 p-2.5 bg-blue-50 border border-blue-200 rounded-lg"
              >
